...
class StrategyMinimax(Strategy):
    ...
    def minimax(self, state):
        ...
        Return a float depending on if there exists a winning strategy for 
        player:
        ...
        if state.winner('p1'):
            ...
            return max([self.minimax(state.apply_move(i)) for i in 
                        state.possible_next_moves()])
        elif state.next_player == 'p2':
            return min([self.minimax(state.apply_move(i)) for i in 
                        state.possible_next_moves()])
        else: 
            return 0
